...
def draw_bars(canvas, x, y, width, height, n):
    ...
    """
    DrawCanvas.draw_rect(canvas, x, y, width, height, color='lightblue')
    for i in range(0, n):
        x_add = (i / (n - 1)) * (width - 1)
        DrawCanvas.draw_line(canvas, x + x_add, y, x + x_add, y + (height - 1), color='black')
...
def draw_eye(canvas, x, y, width, height, n):
    ...
    """
    DrawCanvas.draw_rect(canvas, x, y, width, height, color='lightblue')
    DrawCanvas.fill_oval(canvas, x, y, width, height, color='yellow')
    for i in range(0, n):
        x_add = (i / (n - 1)) * (width - 1)
        DrawCanvas.draw_line(canvas, x + width / 2, y + height / 2, x + x_add, y + (height - 1), color='black')
...
def draw_bowtie(canvas, x, y, width, height, n):
    ...
    for i in range(0, n):
        ...
        DrawCanvas.draw_line(canvas, x + width / 2, y + height / 2, x + (width - 1), y + y_delta, color='red')
    for i in range(0, n):
        y_delta= (i / (n - 1)) * (height - 1)
        DrawCanvas.draw_line(canvas, x + width / 2, y + height / 2, x - 1, y + y_delta, color='red')
